...
## Lock File
- **Name**: `yarn.lock`
- **Auto-resolve command**: `git checkout --ours yarn.lock` (the accumulating
  branch's lock, NOT `--theirs` — `--theirs` in a rebase means the stale
  dependabot branch's own lock, and using it here silently drags previously
  merged package versions backward; see base-merge-logic.md's Mandatory
  Post-Regeneration Verification), then `yarn install` to regenerate, then
  diff the result against the pre-rebase accumulating-branch HEAD to confirm
  no package regressed before continuing the rebase.
