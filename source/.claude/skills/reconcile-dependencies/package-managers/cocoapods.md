...
- **Auto-resolve command**: `git checkout --ours ios/Podfile.lock` (the
  accumulating branch's lock, NOT `--theirs` — see base-merge-logic.md's
  Mandatory Post-Regeneration Verification for why `--theirs` silently
  regresses previously merged pod versions)

**Strategy**: Always accept **ours** for lock file conflicts, then run
`pod install` to regenerate, then diff the regenerated lock against the
pre-rebase accumulating-branch HEAD to confirm no pod version regressed
...
- `yarn install --frozen-lockfile`
- `cd ios && pod install`
- Full xcodebuild with the project's dev scheme
...
**Strategy**: Always auto-resolve, starting from **ours**
```bash
git checkout --ours ios/Podfile.lock
git add ios/Podfile.lock
cd ios && pod install && cd ..
git diff <accumulating-branch-pre-rebase-sha> -- ios/Podfile.lock  # check for regressions
```

The `pod install` will regenerate the lock file based on the merged Podfile.
Starting from `--ours` (not `--theirs`) matters — see base-merge-logic.md's
...
✅ `ios/<App>.xcworkspace` exists and is valid
